...
| End-to-end (full sandbox sync) | Playwright | `npm run test:e2e` + `E2E_GENESYS_SANDBOX` | Yes | Yes (sandbox OAuth client) |
...
externally-dependent full-sync E2E as the release gate.
...
| Auth: session / CSRF | `src/server/auth/__tests__/auth.test.ts` | Session sign/verify; rejects expired, tampered, and wrong-secret tokens; `timingSafeEqual`; single-admin `verifyCredentials`; `requireAuth` rejects no-session and accepts a valid session cookie; `requireCsrf` double-submit match, rejects mismatch and cross-origin mutating requests. |
...
- **UI / navigation / security flows** run against the local dev/preview build
  with **no external dependencies** — page routing, vault lock-on-refresh,
  import/export, protected-route checks, security headers.
- **Full sandbox-sync flow** (the release gate, below) requires a real Genesys
  sandbox and is opt-in.

### Full sandbox-sync E2E (release gate)

The end-to-end sync flow — **create a source, upload one file of every supported
extension (`.txt .md .doc .docx .csv .xls .xlsx .html .pdf`), and complete the
synchronization** — exercises the real four-phase File Connector flow against
Genesys Cloud. Because it needs real credentials and creates real Genesys state,
it is gated behind an environment flag and is **skipped unless explicitly
enabled**:

- Set `E2E_GENESYS_SANDBOX` (and supply a Genesys **sandbox** OAuth Client
  Credentials client via `GENESYS_CLIENT_ID` / `GENESYS_CLIENT_SECRET` /
  `GENESYS_REGION_API_HOST`, plus the app login `ADMIN_USERNAME` /
  `ADMIN_PASSWORD` / `APP_SESSION_SECRET`).
- Use a disposable sandbox org with least-privilege Knowledge permissions; the
  test creates and syncs real `FileUpload` sources.

This is the **release gate**: a production release is not signed off until the
full sandbox-sync E2E passes green.
...
- **Direct upload** to a Genesys pre-signed URL depends on the upload host being
  ...
  direct upload is blocked, the proxy fallback (`ENABLE_PROXY_UPLOAD`,
  `PROXY_UPLOAD_MAX_BYTES`) is the path under test instead.

---

## 3. CI gate

The single pre-merge gate aggregates formatting, lint, types, and the full unit
suite:
